    ...
    if (lineText.startsWith('@')) {
      ...
      for (let t = 0; t < token.length; t++) {
        ...
        if (
          ...
          matches.groups.color == cursor.color
        ) {
...
const updateCursor = (message) => {
  ...
  let end;
  let to = message.to;
  let ccText = '@' + String(to.time) + to.unit;
  ...
  if (message.from == null) {
    ...
    let remain =
      range.line.text.substring(0, start) + range.line.text.substring(end);
    if (remain.trim().length == 0) {
      // This was the only cursor on this line, so remove the whole line,
      // including any comments and the line feed char (thus +1)
      end = range.line.to + 1;
    }
  } else {
