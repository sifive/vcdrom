...
const helpUI = require('./helpUIOpener.js');
const statusBar = require('./statusBar.js');
// const msgbox = require('./messageBox.js');
const ProgressBar = require('./buildSifiveLogo.js');
const actions = require('./doppler/action.js');
...
const getHandler = (content, inst) => async (readers) => {
...

  {
    ...
    if (r) {
      ...
      ProgressBar.buildSifiveLogo();
      let vcdSize = ProgressBar.getVCDsize();
      let percentage;
      let total = 0;
      outerLoop: for (let i = 0; i < 1e5; i++) {
        ...
        for (let j = 0; j < len; j += maxChunkLength) {
          ...
          total += len1;
          percentage = Math.round((total / vcdSize) * 100);
...
helpUI.listenForUI();
statusBar.setupStatusBarListener();
