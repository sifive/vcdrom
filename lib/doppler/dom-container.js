...
const setupMouseHandlers = (cursor, content, pstate) => {
  const moveHandler = (event) => {
    moveCursor(
      !pstate.markmode && !pstate.editMode,
      event,
      cursor,
      pstate,
      true
  ...
  const clickHandler = (event) => {
    moveCursor(!pstate.editMode, event, cursor, pstate, false);
  };
...
const domContainer = (content, desc /* , opt */) => {
  ...
  createHelpUI().addEventListener('click', () => {
    // toggle the help ui
    document.getElementById('openDiv').classList.toggle('show');
    document.getElementById('helpButton').classList.toggle('show');
  });
  ...
  cm.on('blur', (cm) => {
    ...
    action.setEditMode(pstate, cm, false);
  });
  ...
  statusBarMgr.createStatusBars(pstate, cm, ws);
