...
const getTime = (timeStr1) => {
  if (!isNaN(timeStr1)) {
    console.log('o am here workuing ', pstate, timeStr1, cursor, render);
    pstate.xScale = 40;
    gotoTime(parseInt(timeStr1), pstate, cursor, true, render);
    timeStr1 = TIMEPROMPT;
  }
...
const openMessageBox = () => {
  ...
  // msgbox.makeTimedMessage(10000, 'checking');
  // msgbox.createXMessageBox();
  // msgbox.makeXMessageBox('the content');
  // msgbox.createDialogueBox();
  // msgbox.makeDialogueBox('SAMPLE TITLE', 'THIS IS THE BODY', 'n', 'y');
...
const toggleEditMode = (cm) => {
  console.log('sssssss this sio working ');
  console.log('this is the toggle edit mode', cm);
  if (cm.isReadOnly()) {
